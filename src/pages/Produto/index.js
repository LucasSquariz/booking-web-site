import * as React from 'react';
import './style.scss';
import LightBox from '../../components/Lightbox';
import Calendar1 from '../../components/Calendar/calendar1';
import Calendar2 from '../../components/Calendar/calendar2';
import { useParams } from "react-router-dom";
import { Link } from "react-router-dom";
import Politica from '../../components/PoliticaProduto';
import { useEffect, useState } from 'react';
import api from '../../services/api';
import swal from 'sweetalert';

function Produto() {
    const { produto } = useParams();
    const [produtoreserva, setProdutoReserva] = useState({});    

    const logado = JSON.parse(localStorage.getItem('user'));

    const scrollWindow = () => {
        window.scrollTo(0, 0)
    }      
   
    const MensagemErro = () =>{
        swal({  
            title: 'Atenção',
            text: "Você precisa estar logado para fazer uma reserva!",
            icon: "error",
          });
          
    }
    useEffect(() => {        
        api.get(`/produto/${produto}`)  
        .then((response) => {      
            console.log(response.data)      
            setProdutoReserva(response.data)
            localStorage.setItem('produto', JSON.stringify(response.data))
            console.log(response.status)
        })     
        .catch((e) => {
            console.log(e.message)
        })
    },[]);
    
    return (
        <>        
            <div id="produto-page" >
                <div className="container1" onLoad={scrollWindow}>
                    <div> {/* --- Início do cabeçalho --- */}
                        <div className="cabecalho">
                            <div>
                                <span className="categoria-produto">{produtoreserva?.categorias?.nome}</span> | <span className="nome-produto">{produtoreserva.nome}</span>
                            </div>
                        </div>
                        <div className="avaliacao-localizacao">
                            <div className="av">
                                <svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.27778 0L8.91174 4.83688H14.1994L9.92159 7.82624L11.5555 12.6631L7.27778 9.67376L3.00001 12.6631L4.63397 7.82624L0.3562 4.83688H5.64382L7.27778 0Z" fill="#FBC02D" /></svg>
                                <svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.27778 0L8.91174 4.83688H14.1994L9.92159 7.82624L11.5555 12.6631L7.27778 9.67376L3.00001 12.6631L4.63397 7.82624L0.3562 4.83688H5.64382L7.27778 0Z" fill="#FBC02D" /></svg>
                                <svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.27778 0L8.91174 4.83688H14.1994L9.92159 7.82624L11.5555 12.6631L7.27778 9.67376L3.00001 12.6631L4.63397 7.82624L0.3562 4.83688H5.64382L7.27778 0Z" fill="#FBC02D" /></svg>
                                <svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.27778 0L8.91174 4.83688H14.1994L9.92159 7.82624L11.5555 12.6631L7.27778 9.67376L3.00001 12.6631L4.63397 7.82624L0.3562 4.83688H5.64382L7.27778 0Z" fill="#FBC02D" /></svg>
                                <svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.27778 0L8.91174 4.83688H14.1994L9.92159 7.82624L11.5555 12.6631L7.27778 9.67376L3.00001 12.6631L4.63397 7.82624L0.3562 4.83688H5.64382L7.27778 0Z" fill="#FBC02D" /></svg>
                            </div>
                            <div>
                                <svg width="12" height="20" viewBox="0 0 18 26" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9 12.35C8.14752 12.35 7.32995 12.0076 6.72716 11.3981C6.12436 10.7886 5.78571 9.96195 5.78571 9.1C5.78571 8.23805 6.12436 7.4114 6.72716 6.8019C7.32995 6.19241 8.14752 5.85 9 5.85C9.85248 5.85 10.67 6.19241 11.2728 6.8019C11.8756 7.4114 12.2143 8.23805 12.2143 9.1C12.2143 9.5268 12.1311 9.94941 11.9696 10.3437C11.8081 10.738 11.5713 11.0963 11.2728 11.3981C10.9744 11.6999 10.62 11.9393 10.2301 12.1026C9.84008 12.2659 9.42211 12.35 9 12.35ZM9 0C6.61305 0 4.32387 0.958747 2.63604 2.66533C0.948211 4.37191 0 6.68653 0 9.1C0 15.925 9 26 9 26C9 26 18 15.925 18 9.1C18 6.68653 17.0518 4.37191 15.364 2.66533C13.6761 0.958747 11.3869 0 9 0Z" fill="#383B58" /></svg>
                                <span className="localizacao-texto">{produtoreserva?.cidade?.nome}</span>
                            </div>
                        </div>
                    </div>{/* --- Fim do cabeçalho --- */}
                    <div className="galeria-imagens"> {/* --- Início da galeria --- */}
                        <div>
                            <img className="img-principal" src={!!produtoreserva?.imagens ? produtoreserva?.imagens[0].url : ""} alt="principal"></img>
                        </div>
                        <div className="imagens-secundarias">
                            <img className="img-secundaria" src={!!produtoreserva?.imagens ? produtoreserva?.imagens[1].url : ""} alt="principal"></img>
                            <img className="img-secundaria" src={!!produtoreserva?.imagens ? produtoreserva?.imagens[2].url : ""} alt="principal"></img>
                            <img className="img-secundaria" src={!!produtoreserva?.imagens ? produtoreserva?.imagens[3].url : ""} alt="principal"></img>
                            <div className="imagem-botao">
                                <img className="img-secundaria" src={!!produtoreserva?.imagens ? produtoreserva?.imagens[4].url : ""} alt="principal"></img>
                                <LightBox />
                            </div>
                        </div>
                    </div> {/* --- Fim da galeria --- */}
                    <div className="descricao-produto"> {/* --- Início da descrição e localização do produto --- */}
                        <div className="descricao-localizacao">
                            <div id="descricao">
                                <div>
                                    <h3>Descrição</h3>
                                    <p>{produtoreserva.descricao}</p>
                                </div>
                            </div>
                            <div className="mapa-produto">
                                {/* <div>
                                <h3>Localização</h3>
                                <iframe className="mapa" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d58517.64389928515!2d-46.67166959470597!3d-23.555771399999998!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x94ce448183a461d1%3A0x9ba94b08ff335bae!2zU8OjbyBQYXVsbywgU1A!5e0!3m2!1spt-BR!2sbr!4v1648075086681!5m2!1spt-BR!2sbr" width="600" height="450" allowFullScreen="" loading="lazy" referrerPolicy="no-referrer-when-downgrade"></iframe>
                            </div> */}
                            </div>
                        </div>

                    </div> {/* --- Fim da descrição e localização  do produto --- */}

                    <div> {/* --- Início do bloco de datas disponíveis --- */}
                        <div className="div-calendario">
                            <h3>Veja a disponibilidade das datas e faça sua reserva!</h3>
                            <div className="calendario-reserva">
                                <div className="calendario1">
                                    <Calendar1 />
                                </div>
                                <div className="calendario2">
                                    <Calendar2 />
                                </div>
                                <div className="div-btn">
                                    <Link to={logado !== null ? `/produto/reserva/${produto}` : "/login"}>
                                        <button className="btn-reserva" onClick={logado == undefined ? MensagemErro : scrollWindow}> Iniciar reserva</button>
                                    </Link>
                                </div>
                            </div>
                        </div>
                    </div> {/* --- Fim do bloco de datas disponíveis --- */}
                    <div className="caracter"> {/* --- Início das caracteristicas do produto --- */}
                        <h3>Essa acomodação possui</h3>
                        <div className="caracteristicas-produto">
                            <div>
                                <p>
                                    <svg width="30" height="28" viewBox="0 0 30 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M30 16.625L24.375 11.375V3.5H20.625V7.875L15 2.625L0 16.625V17.5H3.75V26.25H13.125V21H16.875V26.25H26.25V17.5H30V16.625Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">4 quartos</span>
                                </p>
                                <p>
                                    <svg width="28" height="28" viewBox="0 0 21 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M2.625 14.8571H20.3438C20.7026 14.8571 21 15.2779 21 15.7857V22.2857H18.375V18.5714H2.625V22.2857H0V4.64286C0 4.13505 0.297363 3.71429 0.65625 3.71429H1.96875C2.32764 3.71429 2.625 4.13505 2.625 4.64286V14.8571ZM8.53125 10.2143C8.53125 8.16853 7.35205 6.5 5.90625 6.5C4.46045 6.5 3.28125 8.16853 3.28125 10.2143C3.28125 12.26 4.46045 13.9286 5.90625 13.9286C7.35205 13.9286 8.53125 12.26 8.53125 10.2143ZM21 13.9286V13C21 9.92411 19.2363 7.42857 17.0625 7.42857H9.84375C9.48486 7.42857 9.1875 7.84933 9.1875 8.35714V13.9286H21Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">3 camas de solteiro</span>
                                </p>
                                <p>
                                    <svg width="28" height="28" viewBox="0 0 25 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M19.9805 6.83203V2.70703H13.7109V6.83203H19.9805ZM11.25 6.83203V2.70703H4.98047V6.83203H11.25ZM22.5 6.83203C23.1641 6.83203 23.7402 7.11133 24.2285 7.66992C24.7168 8.22852 24.9609 8.87305 24.9609 9.60352V16.5H23.3203L22.5 19.207H21.2109L20.3906 16.5H4.57031L3.75 19.207H2.46094L1.64062 16.5H0V9.60352C0 8.87305 0.244141 8.22852 0.732422 7.66992C1.2207 7.11133 1.79688 6.83203 2.46094 6.83203V2.70703C2.46094 1.97656 2.71484 1.34277 3.22266 0.805664C3.73047 0.268555 4.31641 0 4.98047 0H19.9805C20.6445 0 21.2305 0.268555 21.7383 0.805664C22.2461 1.34277 22.5 1.97656 22.5 2.70703V6.83203Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">2 camas de casal</span>
                                </p>
                                <p>
                                    <svg width="25" height="28" viewBox="0 0 25 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M19.2871 9.35156C20.0033 9.35156 20.6136 9.0599 21.1182 8.47656C21.6227 7.89323 21.875 7.20052 21.875 6.39844C21.875 5.92448 21.7448 5.38672 21.4844 4.78516C21.224 4.18359 20.9229 3.60937 20.5811 3.0625C20.2393 2.51562 19.9382 2.0599 19.6777 1.69531L19.2871 1.14844L18.8965 1.69531C18.8965 1.69531 18.5872 2.15104 17.9687 3.0625C17.3503 3.97396 17.041 4.54818 17.041 4.78516C17.041 5.02214 16.9108 5.5599 16.6504 6.39844C16.6504 7.20052 16.9108 7.89323 17.4316 8.47656C17.9525 9.0599 18.571 9.35156 19.2871 9.35156ZM14.0625 10.5C14.4857 10.5 14.8519 10.3268 15.1611 9.98047C15.4704 9.63411 15.625 9.22396 15.625 8.75C15.625 8.49479 15.5436 8.20312 15.3809 7.875C15.2181 7.54687 15.0391 7.22786 14.8437 6.91797C14.6484 6.60807 14.4694 6.35286 14.3066 6.15234L14.0625 5.85156L13.8184 6.15234C13.8184 6.15234 13.6393 6.40755 13.2812 6.91797C12.9232 7.42839 12.7441 7.7474 12.7441 7.875C12.7441 8.0026 12.6628 8.29427 12.5 8.75C12.5 9.22396 12.6546 9.63411 12.9639 9.98047C13.2731 10.3268 13.6393 10.5 14.0625 10.5ZM9.52148 5.85156L2.00195 13.7812C1.35091 14.4375 1.02539 15.276 1.02539 16.2969V23.3516C1.02539 23.9714 1.17187 24.5456 1.46484 25.0742C1.75781 25.6029 2.1403 26.0312 2.6123 26.3594C3.08431 26.6875 3.59701 26.8516 4.15039 26.8516H18.5059C18.8639 26.8516 19.165 26.7057 19.4092 26.4141C19.6533 26.1224 19.7754 25.776 19.7754 25.375C19.7754 24.974 19.6533 24.6276 19.4092 24.3359C19.165 24.0443 18.8639 23.8984 18.5059 23.8984H12.5V22.75H20.5566C20.9147 22.75 21.224 22.6042 21.4844 22.3125C21.7448 22.0208 21.875 21.6745 21.875 21.2734C21.875 20.8724 21.7448 20.5352 21.4844 20.2617C21.224 19.9883 20.9147 19.8516 20.5566 19.8516H12.5V18.6484H21.6309C21.9889 18.6484 22.29 18.5117 22.5342 18.2383C22.7783 17.9648 22.9004 17.6276 22.9004 17.2266C22.9004 16.8255 22.7783 16.4792 22.5342 16.1875C22.29 15.8958 21.9889 15.75 21.6309 15.75H12.5V14.6016H19.5312C19.8893 14.6016 20.1986 14.4557 20.459 14.1641C20.7194 13.8724 20.8496 13.526 20.8496 13.125C20.8496 12.724 20.7194 12.3776 20.459 12.0859C20.1986 11.7943 19.8893 11.6484 19.5312 11.6484H9.22852C9.45638 11.2109 9.69238 10.7643 9.93652 10.3086C10.1807 9.85286 10.3841 9.46094 10.5469 9.13281L10.791 8.64062C10.8887 8.42187 10.9375 8.20312 10.9375 7.98437C10.9375 7.69271 10.8398 7.41927 10.6445 7.16406L10.4736 6.97266C10.4736 6.97266 10.3434 6.81771 10.083 6.50781C9.82259 6.19792 9.69238 6.04297 9.69238 6.04297L9.52148 5.85156Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">3 banheiros</span>
                                </p>
                            </div>
                            <div>
                                <p>
                                    <svg width="28" height="28" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g clipPath="url(#clip0_47_9281)">
                                            <path d="M8.99647 12.7371C8.13616 12.7371 7.43628 13.4347 7.43628 14.2917C7.43628 15.149 8.13616 15.8469 8.99647 15.8469C9.85706 15.8469 10.5569 15.149 10.5569 14.2917C10.5569 13.4347 9.85692 12.7371 8.99647 12.7371Z" fill="#383B58" />
                                            <path d="M12.803 10.6188C11.7872 9.60365 10.4351 9.04431 8.99632 9.04431C7.56304 9.04431 6.21467 9.59996 5.19919 10.6088C4.78906 11.0169 4.78835 11.6819 5.19749 12.0907C5.39624 12.2898 5.66086 12.3991 5.94252 12.3991C6.2229 12.3991 6.48653 12.2903 6.68499 12.093C7.30297 11.4789 8.1238 11.1406 8.99632 11.1406C9.8721 11.1406 10.6952 11.481 11.3136 12.0988C11.5124 12.2973 11.7768 12.4069 12.0582 12.4069C12.3389 12.4069 12.6024 12.2981 12.801 12.1008C13.2117 11.6926 13.2124 11.028 12.803 10.6188Z" fill="#383B58" />
                                            <path d="M17.6928 5.74502C15.3705 3.42848 12.2818 2.15307 8.99615 2.15307C5.71492 2.15307 2.62932 3.4255 0.307779 5.73654C-0.102356 6.14469 -0.10264 6.80965 0.307211 7.21851C0.50582 7.41655 0.770016 7.52543 1.05111 7.52543C1.33205 7.52543 1.59597 7.41697 1.79457 7.21907C3.7189 5.30369 6.27639 4.24861 8.99615 4.24861C11.7199 4.24861 14.2799 5.30582 16.2047 7.22631C16.4033 7.4245 16.6679 7.53381 16.9489 7.53381C17.2297 7.53381 17.4936 7.42478 17.692 7.22716C18.1023 6.8187 18.1027 6.15349 17.6928 5.74502Z" fill="#383B58" />
                                            <path d="M15.186 8.24303C13.5329 6.59382 11.3348 5.68568 8.99632 5.68568C6.66214 5.68568 4.46722 6.59098 2.81561 8.23437C2.61672 8.43212 2.50726 8.69518 2.50684 8.97528C2.50684 9.25509 2.61586 9.51844 2.81447 9.71648C3.01308 9.91466 3.27742 10.024 3.55851 10.024C3.83917 10.024 4.10309 9.91494 4.30198 9.71761C5.55666 8.46932 7.22389 7.78164 8.99632 7.78164C10.7724 7.78164 12.4419 8.47173 13.6975 9.72428C13.8964 9.92275 14.1606 10.0321 14.4418 10.0321C14.7226 10.0321 14.9867 9.92303 15.185 9.72542C15.3836 9.52766 15.4933 9.2646 15.4935 8.98451C15.4938 8.70441 15.3844 8.44135 15.186 8.24303Z" fill="#383B58" />
                                        </g>
                                        <defs>
                                            <clipPath id="clip0_47_9281">
                                                <rect width="18" height="18" fill="white" />
                                            </clipPath>
                                        </defs>
                                    </svg>
                                    <span className="normas-texto">Wi-Fi</span>
                                </p>
                                <p>
                                    <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g clipPath="url(#clip0_47_9274)">
                                            <path d="M20.5488 13.6746C22.0044 13.8449 23.3223 12.803 23.4926 11.3473C23.6628 9.89193 22.621 8.57392 21.1653 8.40367C19.7098 8.23329 18.3918 9.27524 18.2219 10.7309C18.0512 12.1865 19.0934 13.5043 20.5488 13.6746Z" fill="#383B58" />
                                            <path d="M8.99224 11.5167L12.9392 10.7285L13.788 12.0935L10.7129 15.0176C11.1877 15.1376 11.6438 15.3162 12.0677 15.5498C12.271 15.6619 12.4615 15.7913 12.6457 15.9164C12.9425 16.118 13.2217 16.3119 13.4985 16.3879C13.7174 16.448 14.0042 16.4514 14.0224 16.4525C14.4122 16.4491 14.7557 16.3407 15.1063 16.1121C16.0181 15.5176 17.1372 14.9017 18.5252 14.8408C18.6276 14.8363 18.7311 14.834 18.8329 14.834C19.2782 14.834 19.7135 14.8777 20.1299 14.9625C19.8443 14.5065 14.7554 8.3398 14.7554 8.3398L14.6213 8.17936C14.2929 7.78905 13.7433 7.62577 13.2366 7.81146L8.44427 9.56921C7.98223 9.73817 7.69937 10.2231 7.79861 10.7205C7.90837 11.2698 8.44279 11.6263 8.99224 11.5167Z" fill="#383B58" />
                                            <path d="M26.438 17.6477C25.7847 17.9406 25.2978 18.0601 24.7569 18.0601C24.5439 18.0601 24.3236 18.0411 24.0832 18.0017C23.2825 17.8589 22.7118 17.3966 22.1598 16.9494C21.8204 16.6744 21.4997 16.4146 21.1288 16.2234C20.4575 15.8754 19.6501 15.6915 18.7934 15.6915C18.7045 15.6915 18.6141 15.6936 18.5247 15.6975C17.3532 15.7489 16.4034 16.2701 15.5512 16.8257C15.0513 17.1517 14.4583 17.3082 13.9202 17.3082C13.7579 17.3082 13.4275 17.2726 13.2132 17.2108C12.8017 17.0921 12.449 16.8524 12.1078 16.6209C11.9326 16.5018 11.7671 16.3894 11.5998 16.2972C10.8837 15.9024 10.0568 15.6937 9.20853 15.6937H9.19132C8.00372 15.6976 6.92966 16.0618 6.0854 16.7468C4.9098 17.7008 4.13397 17.9689 3.9128 18.0067C3.9128 18.0067 3.72728 18.0552 3.32893 18.0552C2.56581 18.0552 1.92807 17.8052 1.28796 17.5269C0.838279 17.3313 0.414497 17.219 0 17.1863C0.0803683 17.2328 0.157307 17.2813 0.231939 17.3325C0.556842 17.559 0.824263 17.8343 1.08275 18.1006C1.22114 18.243 1.35177 18.3776 1.49057 18.5053C2.03399 19.0052 3.15554 19.5961 4.4464 19.5961C5.10141 19.5961 5.73519 19.4537 6.32994 19.1727C6.50493 19.09 6.6753 18.9915 6.85562 18.8874C7.15479 18.7147 7.46556 18.5407 7.80992 18.4247C8.24447 18.2783 8.91899 18.3268 8.91899 18.3268C9.61836 18.3715 10.1421 18.7019 10.5202 18.9848C11.6722 19.8465 12.7591 20.2532 13.9407 20.2647L14.0009 20.2651C14.814 20.2651 15.5578 20.1083 16.2119 19.7993C16.5166 19.6553 16.8067 19.4435 17.1139 19.219C17.6909 18.7975 18.2876 18.3616 19.056 18.3315C19.1017 18.3297 19.1482 18.3287 19.1954 18.3287C19.6161 18.3287 20.2497 18.4018 20.8785 18.7501C21.6354 19.1707 22.5511 19.6005 23.6076 19.6005C23.7611 19.6005 23.9178 19.5915 24.0733 19.574C25.0458 19.4639 25.8678 19.1054 26.5165 18.5083C26.6556 18.3803 26.7865 18.2453 26.9251 18.1024C27.2208 17.7979 27.5264 17.4829 27.9125 17.2437C27.9415 17.2257 27.9706 17.2082 28 17.1912C27.5333 17.2328 27.039 17.3782 26.438 17.6477Z" fill="#383B58" />
                                        </g>
                                        <defs>
                                            <clipPath id="clip0_47_9274">
                                                <rect width="28" height="28" fill="white" />
                                            </clipPath>
                                        </defs>
                                    </svg>
                                    <span className="normas-texto">Piscina</span>
                                </p>
                                <p>
                                    <svg width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M6.16 0L4.75 1.41L7.34 4H2C0.89 4 0 4.89 0 6V16C0 17.11 0.89 18 2 18H18C19.11 18 20 17.11 20 16V6C20 4.89 19.11 4 18 4H12.66L15.25 1.41L13.84 0L10 3.84L6.16 0ZM2 6H15V16H2V6ZM17.5 6C17.7652 6 18.0196 6.10536 18.2071 6.29289C18.3946 6.48043 18.5 6.73478 18.5 7C18.5 7.26522 18.3946 7.51957 18.2071 7.70711C18.0196 7.89464 17.7652 8 17.5 8C17.2348 8 16.9804 7.89464 16.7929 7.70711C16.6054 7.51957 16.5 7.26522 16.5 7C16.5 6.73478 16.6054 6.48043 16.7929 6.29289C16.9804 6.10536 17.2348 6 17.5 6ZM17.5 9C17.7652 9 18.0196 9.10536 18.2071 9.29289C18.3946 9.48043 18.5 9.73478 18.5 10C18.5 10.2652 18.3946 10.5196 18.2071 10.7071C18.0196 10.8946 17.7652 11 17.5 11C17.2348 11 16.9804 10.8946 16.7929 10.7071C16.6054 10.5196 16.5 10.2652 16.5 10C16.5 9.73478 16.6054 9.48043 16.7929 9.29289C16.9804 9.10536 17.2348 9 17.5 9Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">Smart tv de 50"</span>
                                </p>
                                <p>
                                    <svg width="20" height="16" viewBox="0 0 20 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M20 6H16V3C16 1.34 14.66 0 13 0C11.34 0 10 1.34 10 3H12C12 2.45 12.45 2 13 2C13.55 2 14 2.45 14 3V6H6C7.1 6 8 5.1 8 4V0H2V4C2 5.1 2.9 6 4 6H0V8H2V16H18V8H20V6ZM4 2H6V4H4V2ZM4 14V8H9V14H4ZM16 14H11V8H16V14Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">Cozinha</span>
                                </p>
                            </div>
                            <div>
                                <p>
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M15 17V20H13V17C13 15.9 12.1 15 11 15H8C5.2 15 3 12.8 3 10C3 8.8 3.4 7.8 4.1 6.9C1.8 6.5 0 4.4 0 2C0 1.3 0.2 0.6 0.4 0H2.8C2.3 0.5 2 1.2 2 2C2 3.7 3.3 5 5 5H8V7C6.3 7 5 8.3 5 10C5 11.7 6.3 13 8 13H11C13.2 13 15 14.8 15 17ZM15.9 6.9C18.2 6.5 20 4.4 20 2C20 1.3 19.8 0.6 19.6 0H17.2C17.7 0.5 18 1.2 18 2C18 3.7 16.7 5 15 5H13.8C13.9 5.3 14 5.6 14 6C14 7.7 12.7 9 11 9V11C13.8 11 16 13.2 16 16V20H18V16C18 13.3 16.5 11 14.2 9.8C15.1 9.1 15.7 8.1 15.9 6.9Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">Detector de fumaça</span>
                                </p>
                                <p>
                                    <svg width="19" height="20" viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M18.09 11.95L15.76 12.57L13.76 11.44V8.56L15.76 7.43L18.09 8.05L18.61 6.12L16.84 5.65L17.3 3.88L15.37 3.36L14.75 5.69L12.75 6.82L10.3 5.38V3.12L12.01 1.41L10.59 0L9.3 1.29L8.01 0L6.59 1.41L8.3 3.12V5.38L5.8 6.82L3.8 5.69L3.22 3.36L1.3 3.88L1.77 5.65L0 6.12L0.52 8.05L2.85 7.43L4.85 8.56V11.45L2.85 12.58L0.52 11.96L0 13.89L1.77 14.36L1.3 16.12L3.23 16.64L3.85 14.31L5.85 13.18L8.3 14.62V16.88L6.59 18.59L8.01 20L9.3 18.71L10.59 20L12 18.59L10.3 16.88V14.62L12.8 13.17L14.8 14.3L15.42 16.63L17.3 16.12L16.83 14.35L18.6 13.88L18.09 11.95ZM6.8 8.56L9.3 7.11L11.8 8.56V11.44L9.3 12.89L6.8 11.44V8.56Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">Ar condicionado</span>
                                </p>
                                <p>
                                    <svg width="28" height="28" viewBox="0 0 21 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M6.39092 0.0150431C7.57092 -0.154957 8.82092 1.13504 9.18092 2.91504C9.54092 4.68504 8.89092 6.26504 7.71092 6.44504C6.54092 6.62504 5.28092 5.33504 4.91092 3.55504C4.54092 1.78504 5.21092 0.205043 6.39092 0.0150431ZM13.5409 0.0150431C14.7309 0.205043 15.3909 1.78504 15.0409 3.55504C14.6609 5.33504 13.4109 6.62504 12.2309 6.44504C11.0409 6.26504 10.3909 4.68504 10.7609 2.91504C11.1209 1.13504 12.3709 -0.154957 13.5409 0.0150431ZM1.04092 4.61504C2.18092 4.12504 3.73092 5.01504 4.54092 6.56504C5.30092 8.14504 5.04092 9.80504 3.91092 10.295C2.78092 10.785 1.24092 9.90504 0.450922 8.33504C-0.339078 6.76504 -0.059078 5.09504 1.04092 4.61504ZM19.0409 4.61504C20.1409 5.09504 20.4209 6.76504 19.6309 8.33504C18.8409 9.90504 17.3009 10.785 16.1709 10.295C15.0409 9.80504 14.7809 8.14504 15.5409 6.56504C16.3509 5.01504 17.9009 4.12504 19.0409 4.61504ZM17.3709 15.395C17.4109 16.335 16.6909 17.375 15.8309 17.765C14.0409 18.585 11.9209 16.885 9.93092 16.885C7.94092 16.885 5.80092 18.655 4.04092 17.765C3.04092 17.275 2.35092 15.975 2.48092 14.895C2.66092 13.405 4.45092 12.605 5.51092 11.515C6.92092 10.105 7.92092 7.45504 9.93092 7.45504C11.9309 7.45504 12.9909 10.065 14.3409 11.515C15.4509 12.735 17.3009 13.765 17.3709 15.395Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">Espaço para pets</span>
                                </p>
                                <p>
                                    <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M4 7.875V21C4 21.4839 4.39113 21.875 4.875 21.875H23.25C23.733 21.875 24.125 21.4839 24.125 21V7.875C24.125 7.875 19.1874 6.07075 14.0537 6.07075C9.12925 6.07075 4 7.875 4 7.875ZM19.3125 14.1094C19.3125 16.8884 17.0603 19.1406 14.2812 19.1406C11.5022 19.1406 9.25 16.8884 9.25 14.1094C9.25 11.3304 11.5022 9.07812 14.2812 9.07812C17.0603 9.07812 19.3125 11.3304 19.3125 14.1094ZM23.25 0H4.875C4.39113 0 4 0.391125 4 0.875V7C4 7 9.0855 5.25 14.0537 5.25C19.148 5.25 24.125 7 24.125 7V0.875C24.125 0.391125 23.733 0 23.25 0ZM10.125 3.5H5.75V1.75H10.125V3.5ZM16.6866 3.5C16.2036 3.5 15.8125 3.108 15.8125 2.625C15.8125 2.142 16.2036 1.75 16.6866 1.75C17.1705 1.75 17.5625 2.142 17.5625 2.625C17.5625 3.108 17.1705 3.5 16.6866 3.5ZM19.3125 3.5C18.8286 3.5 18.4375 3.108 18.4375 2.625C18.4375 2.142 18.8286 1.75 19.3125 1.75C19.7955 1.75 20.1875 2.142 20.1875 2.625C20.1875 3.108 19.7955 3.5 19.3125 3.5ZM21.8832 3.5C21.3994 3.5 21.0082 3.108 21.0082 2.625C21.0082 2.142 21.3994 1.75 21.8832 1.75C22.3662 1.75 22.7582 2.142 22.7582 2.625C22.7582 3.108 22.3654 3.5 21.8832 3.5ZM10.1793 14.1094C10.1793 16.3747 12.0159 18.2114 14.2812 18.2114C16.5466 18.2114 18.3832 16.3747 18.3832 14.1094C18.3832 14.1041 16.9798 15.5496 14.2812 13.9449C11.5827 12.341 10.1793 14.1041 10.1793 14.1094V14.1094Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">Maquina de lavar</span>
                                </p>
                            </div>
                            <div>
                                <p>
                                    <svg width="27" height="28" viewBox="0 0 27 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M0.899658 5.83346L8.7749 2.56654V22.1665L0.899658 25.4335V5.83346Z" fill="#383B58" />
                                        <path d="M10.3499 1.75L18.2247 6.65V25.4335L10.3499 21.35V1.75Z" fill="#383B58" />
                                        <path d="M19.8 6.65L26.1002 1.75V21.35L19.8 26.25V6.65Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">Localização de fácil acesso</span>
                                </p>
                                <p>
                                    <svg width="30" height="31" viewBox="0 0 30 31" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M23.7305 3.875H6.26953C5.56641 3.875 4.9707 4.12728 4.48242 4.63184C3.99414 5.13639 3.75 5.75195 3.75 6.47852V24.5215C3.75 25.248 3.99414 25.8636 4.48242 26.3682C4.9707 26.8727 5.56641 27.125 6.26953 27.125H23.7305C24.4336 27.125 25.0293 26.8727 25.5176 26.3682C26.0059 25.8636 26.25 25.248 26.25 24.5215V6.47852C26.25 5.75195 26.0059 5.13639 25.5176 4.63184C25.0293 4.12728 24.4336 3.875 23.7305 3.875ZM10.6055 7.75C11.0352 7.75 11.4062 7.91146 11.7188 8.23437C12.0312 8.55729 12.1875 8.94076 12.1875 9.38477C12.1875 9.82878 12.0312 10.2021 11.7188 10.5049C11.4062 10.8076 11.0352 10.959 10.6055 10.959C10.1758 10.959 9.81445 10.8076 9.52148 10.5049C9.22852 10.2021 9.08203 9.82878 9.08203 9.38477C9.08203 8.94076 9.22852 8.55729 9.52148 8.23437C9.81445 7.91146 10.1758 7.75 10.6055 7.75ZM13.7695 18.1035H12.4805V23.25H8.73047V18.1035H7.5V14.834C7.5 14.1478 7.74414 13.5524 8.23242 13.0479C8.7207 12.5433 9.31641 12.291 10.0195 12.291H11.25C11.9531 12.291 12.5488 12.5433 13.0371 13.0479C13.5254 13.5524 13.7695 14.1478 13.7695 14.834V18.1035ZM19.3945 21.9785L16.2305 16.7715H22.5L19.3945 21.9785ZM16.2305 14.2285L19.3945 9.02148L22.5 14.2285H16.2305Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">Elevador</span>
                                </p>
                                <p>
                                    <svg width="27" height="27" viewBox="0 0 27 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M24.4688 3.375H2.53125C1.13906 3.375 0 4.51406 0 5.90625V21.0938C0 22.4859 1.13906 23.625 2.53125 23.625H24.4688C25.8609 23.625 27 22.4859 27 21.0938V5.90625C27 4.51406 25.8609 3.375 24.4688 3.375ZM2.53125 5.0625H24.4688C24.9261 5.0625 25.3125 5.44888 25.3125 5.90625V8.4375H1.6875V5.90625C1.6875 5.44888 2.07388 5.0625 2.53125 5.0625ZM24.4688 21.9375H2.53125C2.07388 21.9375 1.6875 21.5511 1.6875 21.0938V13.5H25.3125V21.0938C25.3125 21.5511 24.9261 21.9375 24.4688 21.9375ZM3.375 16.875H5.0625V20.25H3.375V16.875ZM6.75 16.875H8.4375V20.25H6.75V16.875ZM10.125 16.875H11.8125V20.25H10.125V16.875Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">Caixa 24h próximo</span>
                                </p>
                                <p>
                                    <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M10.5 25.375C10.5 26.8248 9.32477 28 7.875 28C6.42523 28 5.25 26.8248 5.25 25.375C5.25 23.9252 6.42523 22.75 7.875 22.75C9.32477 22.75 10.5 23.9252 10.5 25.375Z" fill="#383B58" />
                                        <path d="M28 25.375C28 26.8248 26.8248 28 25.375 28C23.9252 28 22.75 26.8248 22.75 25.375C22.75 23.9252 23.9252 22.75 25.375 22.75C26.8248 22.75 28 23.9252 28 25.375Z" fill="#383B58" />
                                        <path d="M28 14V3.5H7C7 2.53351 6.21649 1.75 5.25 1.75H0V3.5H3.5L4.81452 14.7671C4.01352 15.4085 3.5 16.3941 3.5 17.5C3.5 19.4331 5.06702 21 7 21H28V19.25H7C6.03351 19.25 5.25 18.4665 5.25 17.5C5.25 17.494 5.25038 17.4881 5.25044 17.4821L28 14Z" fill="#383B58" />
                                    </svg>
                                    <span className="normas-texto">Mercado próximo</span>
                                </p>
                            </div>
                        </div>
                    </div> {/* --- Fim das caracteristicas do produto --- */}
                    <Politica />
                </div>
            </div>

        </>

    )
}

export default Produto;
